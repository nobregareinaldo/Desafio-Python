...
# API Rest
# /pedidos (GET, POST)
@app.route('/pedidos') # Opcional: ' , methods=['GET'] '
...
def atualizar_pedido(): # Atualizar cadastro do pedido
    try:
        ...
        if _id_ped and _id_cli and _data and _status and _valor and request.method == 'PUT':
            ...
            cursor.execute(sql, data)
            conn.commit()
            resp = jsonify('Pedido ' + _id_ped + " atualizado com sucesso!")
            resp.status_code = 200
            return resp
        else:
            return not_found()
...
@app.route('/clientesTest') # Opcional: ' , methods=['GET'] '
